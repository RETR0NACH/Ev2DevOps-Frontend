...
const onSubmit = async (data) => {
    ...
    try {
      // Petición al MS Ventas
      await axios.put(`${URL_VENTAS}/${venta.idVenta}`, jsonDataSales, {
        headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' }
      });
      
      // Petición al MS Despachos
      await axios.post(`${URL_DESPACHOS}`, jsonData, {
        headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' }
      });
      
      Swal.fire({
        title: "Despacho registrado!",
        text: "Éxito en la base de datos",
        icon: "success"
      });
